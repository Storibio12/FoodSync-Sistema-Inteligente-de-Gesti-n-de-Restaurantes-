...
export default function AdminSalesPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0 mb-8">
        <div>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="text-xs text-slate-500 uppercase bg-slate-50/80 border-b border-slate-200">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 font-medium text-center">Reserva</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                list.map((s) => {
                  const resId = s.reservation_id ?? s.reservationId;
                  return (
                    <tr key={s.sale_id ?? s.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td
                        className="px-6 py-4 text-center text-slate-600 font-mono text-xs"
                        title={resId != null ? `Reserva #${resId}` : undefined}
                      >
                        {resId != null ? `#${resId}` : "—"}
                      </td>
